...
const Display=()=>{
    ...
    const { radius, x, y } = useControls({
        radius: {
            value: loaded.radius, 
            onChange: (v)=>{ refPos.current.radius=v; },
        },
        x:{
            value: loaded.x, 
            onChange: (v)=>{ refPos.current.x=v;},
        },
        y:{
            value: loaded.y,
            onChange: (v)=>{ refPos.current.y=v;},
        },
        save: button(()=>{
    ...
    return (
        <div className="bg-black absolute top-0 left-0 bottom-0 right-0">
            <img src={current} className="fixed rounded-full max-w-none" 
                style={{left: refPos.current.x, top:refPos.current.y, width: refPos.current.radius, height: refPos.current.radius }}/>
            <Leva hidden={!control}/>
        </div>
